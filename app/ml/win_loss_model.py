...
def prepare_training_data(conn=None) -> Tuple[pd.DataFrame, pd.Series]:
...
def train_logistic_regression(X: pd.DataFrame, y: pd.Series) -> Tuple[LogisticRegression, StandardScaler, Dict[str, Any]]:
    """训练逻辑回归基线模型
...
def train_xgboost_model(X: pd.DataFrame, y: pd.Series) -> Tuple[xgb.XGBClassifier, Optional[StandardScaler], Dict[str, Any]]:
    """训练XGBoost分类模型
...
def save_model(model: Any, scaler: Optional[StandardScaler], model_name: str) -> str:
    """保存模型和标准化器到文件
    ...
    save_data = {"model": model, "scaler": scaler, "feature_columns": FEATURE_COLUMNS}
...
def load_model(model_name: str) -> Tuple[Any, Optional[StandardScaler], List[str]]:
    ...
    return save_data["model"], save_data.get("scaler"), save_data.get("feature_columns", FEATURE_COLUMNS)
...
def train_and_save_all_models() -> Dict[str, Any]:
    ...
    conn = db.get_connection()
    try:
        X, y = prepare_training_data(conn)
    finally:
        conn.close()
    ...
    # 训练逻辑回归
    lr_model, lr_scaler, lr_metrics = train_logistic_regression(X, y)
    save_model(lr_model, lr_scaler, "win_loss_logistic")
    results["logistic_regression"] = lr_metrics

    # 训练XGBoost
    xgb_model, _, xgb_metrics = train_xgboost_model(X, y)
    save_model(xgb_model, None, "win_loss_xgboost")
